...
/// transforms at render time. They may also carry a clip shape and a
/// composite mode.
#[derive(Default, Debug, Clone)]
pub struct Group {
    /// Transform accumulated with parent group transforms at render
    /// time.
...
/// Controls whether fill or stroke is drawn first for a
/// [`KanvaPath`].
#[derive(Default, Debug, Clone, Copy, PartialEq, Eq)]
pub enum PaintOrder {
    /// Fill is drawn before stroke (default, matches SVG
    /// `fill-and-stroke`).
    #[default]
    FillStroke,
    /// Stroke is drawn before fill (matches SVG `stroke-and-fill`).
    StrokeFill,
}

/// A stored path with its world transform and optional fill/stroke
/// indices.
#[derive(Default, Debug, Clone)]
...
/// Command index range for a group (parallel to [`crate::Kanva`]'s
/// `groups` vec).
///
/// The inner commands are at `start + 1..end`: `start` is the
/// `PushGroup` and `end` is the `PopGroup`, both excluded when
/// iterating contents.
#[derive(Debug, Clone, Copy)]
