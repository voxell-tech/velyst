...
use kanva::imaging::kurbo::{
    Affine, BezPath, Cap, Join, Point, Shape as _, Stroke,
};
use kanva::imaging::peniko::{
    BlendMode, Blob, Brush, ColorStop, Extend, Fill, Gradient,
    ImageAlphaType, ImageBrush, ImageData, ImageFormat, ImageQuality,
    Mix, Style,
};
use kanva::{Group, KanvaClip, KanvaFill, KanvaSink, KanvaStroke};
use usvg::{Node, PaintOrder};

/// Walk a usvg [`usvg::Tree`] and emit all draw commands into `sink`.
///
/// `transform` is the root world transform applied to all paths (e.g. the
/// accumulated Typst frame transform + any SVG→display scale).
pub fn render_svg(
    tree: &usvg::Tree,
...
pub fn render_path(
    ...
    let fill = lower_fill(path.fill());
    let stroke = lower_stroke(path.stroke());

    match path.paint_order() {
        PaintOrder::FillAndStroke => {
            sink.draw_path(geometry, transform, fill, stroke);
        }
        // Kanva draws fill-then-stroke; for stroke-first, split into two paths.
        PaintOrder::StrokeAndFill => {
            ...
            if fill.is_some() {
                sink.draw_path(geometry, transform, fill, None);
            }
        }
    }
}
...
/// Merge a (possibly chained) clip path into a single [`KanvaClip`] by
/// concatenating all sub-paths. Each sub-path is already in world space
/// (transforms baked in by [`lower_single_clip`]), so simple path
/// concatenation correctly represents the intersection region for the
/// common case of non-overlapping clip shapes.
fn merge_clip_chain(
...
fn collect_clip_chain(
    ...
    if let Some(clip) = lower_single_clip(clip_path, transform) {
...
// --- Fill / stroke lowering ---

fn lower_fill(fill: Option<&usvg::Fill>) -> Option<KanvaFill> {
    let fill = fill?;
    let (brush, brush_transform) =
        lower_brush(fill.paint(), fill.opacity().get())?;
    Some(KanvaFill {
...
fn lower_stroke(
    ...
    let stroke = stroke?;
    let (brush, brush_transform) =
        lower_brush(stroke.paint(), stroke.opacity().get())?;
    Some(KanvaStroke {
