...
use syn::token::Comma;
use syn::{parse_macro_input, DeriveInput, Ident, Index, LitInt, Result};
...
#[proc_macro]
pub fn all_tuples_with_indices(input: TokenStream) -> TokenStream {
    ...
    let invocations = (input.start..=input.end).map(|i| {
        ...
        let number_token = (0..i)
            .map(|i| TokenStream2::from_str(&format!("{i}")).unwrap())
            .collect::<Vec<_>>();
        quote! {
            #macro_ident!({ #(#number_token),* }, #(#ident_tuples),*);
        }
    });
